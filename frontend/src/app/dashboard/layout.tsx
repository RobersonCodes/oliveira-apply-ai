...
const navItems = [
  ...
  { href: '/dashboard/indeed', icon: Globe, label: 'Indeed Global' },
  { href: '/dashboard/geekhunter', icon: Zap, label: 'GeekHunter' },
];

const aiModules = [
  { href: '/dashboard/recruiter-vision', icon: Shield, label: 'Recruiter Vision', badge: 'PRO' },
  { href: '/dashboard/shadow-apply', icon: Ghost, label: 'Shadow Apply', badge: 'PRO' },
  { href: '/dashboard/vaga-radar', icon: Radar, label: 'Vaga Radar', badge: 'PRO' },
  { href: '/dashboard/conexao-cirurgica', icon: Network, label: 'Conexão Cirúrgica', badge: 'PRO' },
];
    ...
    <div className="min-h-screen bg-[#080812] flex">
      ...
      `}>
        ...
        <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto scrollbar-none">
          <div className="text-[10px] text-white/25 font-medium uppercase tracking-widest px-3 py-2">Automação</div>
          {navItems.map(item => <NavLink key={item.href} {...item} />)}

          <div className="text-[10px] text-white/25 font-medium uppercase tracking-widest px-3 py-2 mt-3">Módulos IA</div>
          {aiModules.map(item => <NavLink key={item.href} {...item} />)}

          <div className="text-[10px] text-white/25 font-medium uppercase tracking-widest px-3 py-2 mt-3">Gestão</div>
        ...
        <div className="p-3 border-t border-white/[0.06]">
          <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] transition-colors cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium text-white truncate">{firstName}</div>
              <div className="text-xs text-white/40 truncate">Plano {planLabel}</div>
            </div>
