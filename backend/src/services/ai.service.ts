...
Write a concise, impactful cover letter (max 3 paragraphs). No placeholder text.`;
  ...
  async adaptResume(input: ResumeAdaptInput): Promise<{ adaptedContent: string; addedKeywords: string[]; improvements: string[]; atsScore: number }> {
...
{
  "adaptedContent": "<full adapted resume text in Brazilian Portuguese>",
  "addedKeywords": ["keyword1", "keyword2"],
  "improvements": ["Melhoria aplicada 1", "Melhoria aplicada 2", "Melhoria aplicada 3"],
  "atsScore": <number 0-100>
}`;

    const raw = await this.complete(prompt, system, 1500);
    try {
      const clean = raw.replace(/```json|```/g, '').trim();
      const parsed = JSON.parse(clean);
      return {
        adaptedContent: parsed.adaptedContent || input.resumeContent,
        addedKeywords: parsed.addedKeywords || [],
        improvements: parsed.improvements || [],
        atsScore: parsed.atsScore ?? 70,
      };
    } catch {
      return { adaptedContent: input.resumeContent, addedKeywords: [], improvements: [], atsScore: 70 };
    }
  }
